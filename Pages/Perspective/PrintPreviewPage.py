...
class ChromePrintPreviewPage:
    ...
    def click_cancel_button(self, timeout: float = 1) -> None:
        """
        ...
        :param timeout: The amount of time (in seconds) to wait for the print preview tag to appear before
            returning.

        """
        self._print_preview_app.find(timeout=timeout).shadow_root.find_element(*self.SIDE_BAR_LOCATOR) \
    ...
    def print_preview_page_is_open(self, timeout: float = 1) -> bool:
        """
        Checks if the print preview page is open by attempting to find the print-preview-app tag in the DOM.

        :param timeout: The amount of time (in seconds) to wait for the print preview tag to appear
            before returning.

        :returns: True, if the print-preview-app tag can be located within the timeout window - False otherwise.
        """
        try:
            return self._print_preview_app.find(timeout=timeout) is not None
